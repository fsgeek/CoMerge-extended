...
for b in $BENCH
do
    echo perf stat -B -d -d -d -e ref-cycles numactl --cpunodebind=$node --membind=$node $b >>$DRAM_FILE 2>&1
    perf stat -B -d -d -d -e ref-cycles numactl --cpunodebind=$node --membind=$node $b >>$DRAM_FILE 2>&1
    echo LD_PRELOAD=libvmmalloc.so.1 perf stat -B -d -d -d -e ref-cycles numactl --cpunodebind=$node --membind=$node $b >>$PMEM_FILE 2>&1
    LD_PRELOAD=libvmmalloc.so.1 perf stat -B -d -d -d -e ref-cycles numactl --cpunodebind=$node --membind=$node $b >>$PMEM_FILE 2>&1
done
